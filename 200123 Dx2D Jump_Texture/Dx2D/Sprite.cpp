...
{
	//for (PTVertex& v : m_vecVertex)
	//{
	//	//v.UV = { 0,0 };
	//	v.UV.x = 1.0f - v.UV.x;
	//	//v.UV.y = 1.0f - v.UV.y;
	//}
	for (int i = 0; i < m_vecVertex.size(); i+=4)
	{
		swap(m_vecVertex[i].UV.x, m_vecVertex[i+2].UV.x);
		swap(m_vecVertex[i+1].UV.x, m_vecVertex[i+3].UV.x);
	}
}
